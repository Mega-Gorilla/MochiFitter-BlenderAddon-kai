...
function Show-ProjectList {
    ...
    foreach ($project in $Projects) {
        ...
        if ($status.HasMochiFitter) {
            ...
            $statusText = if ($status.SmoothingProcessorOptimized) {
                "[最適化済み]"
            } else {
                "[未最適化]"
            }

            $statusColor = if ($status.SmoothingProcessorOptimized) { "Green" } else { "Yellow" }
...
function Install-Optimization {
    ...
    try {
        ...
        if ($result.Success) {
            Write-SuccessMessage "パッチを適用しました (適用数: $($result.PatchesApplied))"
            return $true
        } else {
            Write-ErrorMessage $result.Error
            return $false
        }
    }
    catch {
        Write-ErrorMessage "パッチ適用中にエラーが発生しました: $_"
        return $false
    }
}
...
function Uninstall-Optimization {
    ...
    try {
        ...
        if ($result.Success) {
            Write-SuccessMessage "パッチをアンインストールしました"
            return $true
        } else {
            Write-ErrorMessage $result.Error
            return $false
        ...
        Write-ErrorMessage "アンインストール中にエラーが発生しました: $_"
        return $false
    }
}
...
# インストール/アンインストール実行
if ($Uninstall) {
    if (-not $selected.Status.SmoothingProcessorOptimized) {
    ...
    if ($selected.Status.SmoothingProcessorOptimized) {
