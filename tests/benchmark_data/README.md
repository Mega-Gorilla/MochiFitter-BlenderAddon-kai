...
**注意**: このフォルダー内の大きなファイル（*.npz, *.zip等）はgitで追跡されません。
...
純正MochiFitterアドオンのZIPファイル。ベンチマーク比較に使用。

**入手方法**:
- BOOTHから購入・ダウンロード
- https://booth.pm/ja/items/6051400

**配置**:
```
benchmark_data/MochiFitter-BlenderAddon1.0.4.zip
```
...
cp /path/to/MochiFitter-BlenderAddon1.0.4.zip tests/benchmark_data/
```
