...
| 🔴 高 | BMesh メモリリーク | 3関数 | メモリ不足 | ✅ 修正済み (PR #38) |
...
> **ステータス**: PR #38 で修正済み。すべての関数に `.free()` 呼び出しを追加。

`bmesh.from_mesh()` で生成した BMesh オブジェクトが解放されていない箇所があります。

| 関数 | 未解放の BMesh | 修正内容 |
|------|---------------|----------|
| `create_hinge_bone_group` | `cloth_bm` | try/finally で `.free()` 追加 |
| `transfer_weights_from_nearest_vertex` | `body_bm`, `cloth_bm` | 関数末尾に `.free()` 追加 |
| `transfer_weights_x_projection` | `template_bm`, `target_bm` | 関数末尾に `.free()` 追加 |
