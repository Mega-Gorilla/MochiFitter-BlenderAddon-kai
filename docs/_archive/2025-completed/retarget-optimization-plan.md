...
> ⚠️ **重大な発見（2025-12-31）:**
> - **オリジナル MochiFitter は 265.32秒**（上記の287秒はリファクタリング後の値）
> - Phase 1 実装後は **293.97秒（+10.8% 回帰）**
> - **現時点の最優先目標は「オリジナル以下（265秒）に戻すこと」**
> - 詳細は「2.1 ベンチマーク結果」セクションを参照
...
| 日時 | バージョン | 処理時間 | 備考 |
|------|-----------|----------|------|
| 2025-12-30 11:52 | Phase 2 完了 | **287.62秒** | リファクタリング後ベースライン |
...
| **2025-12-31** | **P1-0 + P1-1 + P1-2** | **293.97秒** | **最適化 Phase 1 完了（PR#52 マージ後）** |

#### Phase 1 最適化後ベースライン（2025-12-31 実測）
...
> **Note**: このベンチマークは P1-0（foreach_get）、P1-1（KDTree キャッシュ）、P1-2（Numba JIT）を
> すべて統合した状態での計測結果です。ただし、**Numba 未インストール環境**での実行のため、
> P1-2 は NumPy フォールバックで動作しています。

#### ⚠️ オリジナル MochiFitter との比較（2025-12-31 発見）

**重大な発見: Phase 1 最適化版は、オリジナルより遅い**

| バージョン | 平均処理時間 | 差分 |
|-----------|-------------|------|
| **オリジナル MochiFitter（#48 前）** | **265.32秒** | ベースライン |
| Phase 1 完了（現在の master） | 293.97秒 | **+10.8% 低速** |

**オリジナル MochiFitter ベンチマーク詳細:**
| Run | 処理時間 |
|-----|----------|
| 1 | 278.87秒 |
| 2 | 258.75秒 |
...
> オリジナルスクリプトは `tests/baseline/retarget_script2_14_original.py` に保存。
> 詳細は `tests/baseline/README.md` を参照。

#### 回帰の根本原因

**P1-0 (foreach_get) の副作用:**

`foreach_get` は numpy 配列を返すが、`inverse_bone_deform_all_vertices()` では
Blender の Matrix 演算のために Vector 型が必要。現在の実装では全頂点を Vector に変換しており、
このオーバーヘッドが foreach_get の高速化を相殺している。

```python
# 問題のあるコード
vertices_np = get_mesh_vertices_foreach(mesh_obj.data)  # numpy配列を取得
vertices = [Vector(v) for v in vertices_np]  # ← 全頂点変換のオーバーヘッド
```

**修正方針:**
| 選択肢 | 説明 | リスク |
|--------|------|--------|
| A: numpy 完全移行 | 行列演算を numpy のみで実装 | 中（コード量多） |
| B: 選択的 foreach_get | Blender 型が必要な関数では foreach_get 不使用 | 低（部分的変更） |
...
```
Phase 1 (低リスク・即効):
├─ [x] P1-0: foreach_get + NumPy batch ★★★最優先 (~20x ※ミクロベンチ参考値) - PR#50, PR#52
├─ [x] P1-1: KDTree共有による再構築回避 (+40-65%) - PR#51, PR#52
├─ [x] P1-2: Numba JIT による距離計算高速化（オプション）(+20-30%) - PR#52
├─ [ ] P1-3: スムージング処理のベクトル化（条件付き）(+0-35%)
└─ [x] ベンチマーク・回帰テスト - 293.97秒（平均）達成
...
| 2025-12-31 | 1.8 | **Phase 1 完了**: P1-0/P1-1/P1-2 実装完了（PR#52）、ベースライン計測結果追加（293.97秒）、ロードマップ更新 |
| 2025-12-31 | 1.9 | **回帰発見**: オリジナル MochiFitter（265.32秒）との比較追加、P1-0 の Vector 変換問題を特定、目標を「オリジナル以下」に修正 |
